...
def create_car(world, offset, wheel_radius, wheel_separation, density=1.0, 
    ...
    chassis=world.create_dynamic_body(
        ...
        fixtures=b2.Fixture(
                    shape=b2.Polygon(chassis_vertices),
                    density=density,
                    )
        )
...
class Car (Framework):
    ...
    def post_step(self):
        framework.SCREEN_OFFSETX = -(self.car.position.x - 20)
        self.print_("frequency = %g hz, damping ratio = %g" % (self.hz, self.zeta))
